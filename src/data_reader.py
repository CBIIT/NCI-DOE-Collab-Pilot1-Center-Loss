...
def load_test_cell(path,ctype) : 
    print "huh",path,ctype
    df = pd.read_csv(path,dtype={'CELLNAME':str})
    print("debug type",ctype)
    df = df[ (df['CELLNAME'].str.contains(ctype+":")) ]
    print df
    return df
...
def load_dose_response(path, min_logconc=-5., max_logconc=-5., subsample=None):
    ...
    print "check this key search",df.shape
    #df = df[ (df['CELLNAME'] == 'OV:OVCAR_4') | (df['CELLNAME'] == 'OV:OVCAR_5')]
    print "check this key search",df.shape
...
class RegressionDataGenerator(object):
    ...
    def __init__(self, val_split=0.2, shuffle=True, drug_features='descriptors',
        ...
        server=''
        dir='/p/lscratchf/allen99/anlftp/ftp.mcs.anl.gov/pub/candle/public/benchmarks/P1B3'

        #cell_expr_path = get_file('P1B3_cellline_expressions.tsv', origin=server+'P1B3_cellline_expressions.tsv', cache_subdir=dir)
        drug_desc_path = get_file('P1B3_drug_descriptors.tsv', origin=server+'P1B3_drug_descriptors.tsv', cache_subdir=dir)
        #drug_auen_path = get_file('P1B3_drug_latent.csv', origin=server+'P1B3_drug_latent.csv', cache_subdir=dir)
        dose_resp_path = get_file('P1B3_dose_response.csv', origin=server+'P1B3_dose_response.csv', cache_subdir=dir)
        #test_cell_path = get_file('P1B3_test_celllines.txt', origin=server+'P1B3_test_celllines.txt', cache_subdir=dir)
        test_cell_path = get_file('cell_lines.txt', origin=server+'P1B3_test_celllines.txt', cache_subdir=dir)
        test_drug_path = get_file('P1B3_test_drugs.txt', origin=server+'P1B3_test_drugs.txt', cache_subdir=dir)

        #self.df_cellline = load_cellline_expressions(cell_expr_path, ncols=feature_subsample, scaling=scaling)

        print("logconc:",min_logconc,max_logconc)
...
#
        ...
        # NCI-60 cell types
        ctypes=['BR','CNS','CO','LC','LE','ME','OV','PR','RE']
        ...
        for ctype in ctypes :
           df_test_cell = load_test_cell(test_cell_path,ctype)

           df_test = df.merge(df_test_cell, on='CELLNAME').merge(df_test_drug, on='NSC')
           print('Set aside {} rows as test data'.format(df_test.shape[0]))
           ...
           print("Category cutoffs",category_cutoffs)
           if category_cutoffs is not None:
               ...
               print('Category cutoffs: {}'.format(category_cutoffs))
               print('Dose response bin counts:')
               ...
               print('  Total: {:9d}'.format(len(growth)))
           ...
           print('Rows in train: {}, val: {}, test: {}'.format(self.n_train, self.n_val, self.n_test))
   
           tb,te=(0,(nrows-self.n_val))
           vb,ve=((nrows-self.n_val),nrows)
           sb,se=(nrows,nrows+self.n_test)
           print('Ranges: {} to {} , {} to {} , {} to {}'.format(tb,te,vb,ve,sb,se))
           bname='growth'
           dset=['train','val','test']
           self.df_response.iloc[tb:te][['NSC','GROWTH','CELLNAME']].to_csv(bname+"."+dset[0]+"."+ctype+".csv",sep='\t')
           self.df_response.iloc[vb:ve][['NSC','GROWTH','CELLNAME']].to_csv(bname+"."+dset[1]+"."+ctype+".csv",sep='\t')
           self.df_response.iloc[sb:se][['NSC','GROWTH','CELLNAME']].to_csv(bname+"."+dset[2]+"."+ctype+".csv",sep='\t')
   
           bname='descrip'
           cvals=['CELLNAME'] 
           cvals += list(self.df_drug_desc.columns.values)
           self.df_response.iloc[tb:te].to_csv(bname+"."+dset[0]+"."+ctype+".csv",sep='\t')
           self.df_response.iloc[vb:ve].to_csv(bname+"."+dset[1]+"."+ctype+".csv",sep='\t')
           #self.df_response.iloc[vb:ve][cvals].to_csv(bname+"."+dset[1]+"."+ctype+".csv",sep='\t')
           self.df_response.iloc[sb:se].to_csv(bname+"."+dset[2]+"."+ctype+".csv",sep='\t')
    ...
    ## so far this is being excuted so extend this...
    def tme(self, data, bname):
        print('what the hell world1')
        if data == 'val':
            cyc = self.cycle_val
        elif data == 'test':
            cyc = self.cycle_test
        ...
        print('made it here world2')
        huh=islice(cyc,0,None)
        #sys.exit(0)
        indices = list(islice(cyc, 0, None))
        print("Make it here?")
    ...
    def mslice(self, data, bname):
        print('made it here world1')
        if data == 'val':
            cyc = self.cycle_val
        elif data == 'test':
            cyc = self.cycle_test
    ...
    def sliceme(self, batch_size, data,topology):
